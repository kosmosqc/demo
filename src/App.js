...
function App() {
  ...
  if (load === 0) {
    return (
      <div class="main">
        <div >
          <button class="click" onClick={() => setLoad(load + 1)}>
            Click me!
          </button>
        </div>
            ...
            <BrowserRouter>
              <h1 style={{ display: "flex", margin: "10px", height: h, width: "150px" }}>
                <Menu  style={{ backgroundColor: "#5f43b2" }}>
                  <Grid >
                    <Grid.Column  >
                      <Menu.Item as={NavLink } onClick={() => setLoad(load - 1)} style={{ color: "#96c2ff", fontSize: size }} activeStyle={{ fontWeight: "bold" }} to="/about" exact={true}>Accueil   </Menu.Item>
                      <Menu.Item as={NavLink}  style={{ color: "#96c2ff", fontSize: size }} activeStyle={{ fontWeight: "bold", color: "white" }} to="/About" exact={true}>About   </Menu.Item>
